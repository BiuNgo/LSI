...
module tb_cubic_solver();
    ...
    task run_test;
        ...
        begin
            ...
            cycles = 0;
            while (valid == 1'b0 && cycles < 5000) begin
                @(posedge clk);
                cycles = cycles + 1;
            end

            if (cycles >= 5000) begin
                $display(">>> [FAIL] TIMEOUT ERROR: FSM got stuck and never asserted valid! <<<");
            end else if (error) begin
                $display(">>> [PASS] ERROR FLAG ASSERTED (Invalid Equation caught safely) <<<");
                $display("    Calculation aborted in %0d clock cycles.", cycles);
            end else begin
                ...
                $display(">>> [PASS] Calculation completed in %0d clock cycles.", cycles);
            end
            $display("---------------------------------------------------------\n");
